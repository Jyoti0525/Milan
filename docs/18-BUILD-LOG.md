...
## Day 10 - the model, priced and plotted

Day 8 measured one model on one tier and published the result. This turns that
point into a curve, puts a measured price on it, and makes it replayable by a
reader with no GPU.

### The published ablation had gone stale, and nothing could have told us

The README said **77 shortfalls, 19.5% agreement, 26 rejected**. A fresh run
says **110 shortfalls, 16.4% agreement, 47 rejected**.

Nothing broke. The shortfall rung landed after that table was written, and it
attributes 43 more credits - each of which becomes a shortfall the triage is
asked about. The population grew by 43 harder cases and the agreement rate
fell accordingly. Both numbers were true when published; only one of them was
true today.

Every other number in the README is regenerated by a command and asserted by a
test. This one was typed, because it needs a model - which is exactly the
argument for the cache.

### The cache could not tell two models apart

Found while building the model-size comparison, and it would have produced a
finding rather than an error.

A caller never names a model. It asks a provider a question, and the provider
uses whichever model it was configured with - so `Request.model` was empty on
every real request, and the model was absent from the cache key. Two models
answering the same question shared one entry, and the second silently replayed
the first one's answer.

The only place that surfaces is the benchmark this cache exists to make
reproducible: two columns of identical figures, reading as *model size does
not matter here* instead of as a bug. `CachedProvider` now fills the model in
before hashing, and a test puts two providers with different models through
one cache and asserts both were asked.
...
| rejected by arithmetic | 0 | 47 |
| identifiers invented | 0 | 5 |

The 1.5B model returned valid schema on all 110 questions and answered
`unknown` to every one. Not a parse failure - it declined, correctly formatted,
110 times. A column of zeros that reads as caution and is really absence.

### Run it twice, two different books

The demo the build order asked for, measured rather than asserted:
...
The first row was a surprise worth keeping: greedy decoding is stable without
any seed at all. The second says a pinned seed makes even sampling repeat
itself. The third is the default configuration of almost every LLM integration
anyone writes, and it produced two different answers to eleven of thirty
identical questions - four of them blaming a different refund.

So the honest claim is not "models are random". It is that **reproducibility
with a model is conditional, and the conditions are not the defaults.** Gemini
offers no seed parameter at all.

This runs without the cache in front of it, deliberately. Everywhere else a
cached answer is what makes a run with a model reproducible; here a cache hit
would prove the point by refusing to run the experiment.

### Cost, measured on the volume and assumed on the rate

Token counts come from the providers' own counters - `prompt_eval_count` and
`eval_count` on Ollama, `usage` on Groq, `usageMetadata` on Gemini - rather
than from an estimate over characters. 66,146 in and 2,453 out for the full
ablation.

Actual spend: **Rs 0.00**. Projected at published rates: **$0.0076** on Gemini
2.0 Flash, **$0.0410** on Groq's Llama 3.3 70B. Each rate is stored with its
source and the date it was read, because a price is somebody else's claim and
it goes stale - and one of the two is a secondary source, which is recorded as
one rather than presented as the vendor's word.

The counts are stored in the cache, so a replayed run reports the same cost as
the run that paid for it. A cache that dropped them would make a replay look
free, which is the direction an error is least likely to be questioned in.

### The degradation curve

| Measure | clean | realistic | messy | adversarial |
|---|---|---|---|---|
| match rate | 100.0% | 100.0% | 100.0% | 100.0% |
| settlement attributed | 100.0% | 99.6% | 99.3% | 98.0% |
| shortfalls named | - | 96.7% | 95.0% | 91.7% |
| leaks caught | - | - | 100.0% | 100.0% |

Three measures show a dash on the clean tier rather than a flattering 100%,
because it generates nothing impossible, nothing merged and nothing mispriced.
A rate over an empty denominator is not a measurement - the same rule that put
a dash on the **Refused** card in the queue yesterday.

Match rate and precision are flat, which is a fact about this problem rather
than a compliment: a settlement total is close to a unique fingerprint once the
fee stack is modelled. The two rates that move are the two where the evidence
genuinely thins.
